...
| `AnnotatedScrollBar.cs` | XAML: `AnnotatedScrollBar/AnnotatedScrollBar.xaml`, `_themeresources.xaml`, `_perf2026.xaml`. Source: `AnnotatedScrollBar.cpp`, `.h`, `.idl`, label/panning/scrolling event args. Generated: `AnnotatedScrollBar.properties.*`, `AnnotatedScrollBarLabel.properties.cpp`. | Live two-way scroll integration through `IScrollController`; absolute labels and independent dense ticks; one `RailMetrics` geometry oracle; debounced collision collapse; pointer detail flag; click/drag/repeat-button and Home/End/Page/arrow keyboard input; `TemplateParts` element factories; automation scrollbar role. The gallery drives a real 9,000-row `ItemsView`. | FluentGpu intentionally exposes compact options/delegates instead of WinUI dependency properties and event-args classes. The remaining platform-facing delta is a numeric RangeValue/scroll-percentage automation provider and its value-change events; visual and input mechanics are live, not a static template shell. |
| `SemanticZoom.cs` | XAML/source: `SemanticZoom/`, `SemanticZoom_Partial.h/.cpp`, generated SemanticZoom properties. | Reusable retained two-view control with controlled state, controller verbs, bidirectional item-index anchoring, lifecycle callbacks, focus/Escape handling, scroll preservation and directional scale+opacity transitions. | FluentGpu uses explicit `SemanticZoomView` slots and optional `ItemsViewController` handles instead of WinUI's `ISemanticZoomInformation`/`IListViewBase`; the anchoring behavior is the parity contract. |
