...
| `CS0117: 'TextEl'/'ImageEl'/… does not contain a definition for 'HitTestVisible'` (or `OnClick`/`Focusable`/`Role`/`Cursor`/`Draggable`/`DropTarget`/`Repeats`/`TabStop`/…) | Interaction properties are declared only on `BoxEl`, not the abstract `Element` base — leaf element types (`TextEl`, `SpanTextEl`, `ImageEl`, `GridEl`, `ScrollEl`, `PolylineStrokeEl`, `IconLayerEl`, `IconGlyphEl`) don't have them | Put `Role`/`Focusable`/`Cursor`/`OnClick`/etc. on the enclosing `BoxEl`, not the leaf child. You almost never need to touch hit-testing on the child at all: cursor and hover-color resolution **walk up the ancestor chain** to the nearest node with an explicit `Cursor`, so a bare `TextEl` inside a `Cursor=Hand` Box "just works" — give the text only `Color`/`HoverColor`(/`PressedColor`), and the click/cursor/focus behavior on the Box. (Established idiom: `StageChrome.PivotLink`, `ContextBand.Link`, `LibraryPage.TitleLink`.) |
