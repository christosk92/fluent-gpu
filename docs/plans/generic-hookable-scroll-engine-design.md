...
> **Note (touchpad axis re-latch — sibling latch, dispatcher-owned).** Distinct from the direction latch above but sharing its "resist a jittery flip" intent: the dispatcher's *axis* latch — the X-vs-Y dominant-axis pick run over `PanSlopPx` at each `ScrollBegin` — now carries a **sticky bias across segment restarts**. A touchpad segments one continuous scroll into several begin/end cycles, so within ~400 ms of the previous gesture's end the pick requires 2× dominance to switch axes; a briefly cross-axis window during a fast vertical drag no longer latches a horizontal shelf for a segment, while a deliberate horizontal swipe still clears the 2× threshold. This lives in `AccumulateContactDelta`/`EndScrollGesture`, not the bind layer.
