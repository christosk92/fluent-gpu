...
- **Step 5 — the async flip: CODE LANDED (2026-07-01) but gated `FG_RENDER_ASYNC`, default-OFF, and EMPIRICALLY confirmed unsafe-to-enable.** `RenderThread` async mode + the non-blocking `WakeAsync` publish are implemented, and the arena reuse is safe by construction (the publisher owns 3 per-slot arenas selected by `PickFreeSlot`, so the UI never writes the slot the render thread reads). But enabling it + running `--screenshot` crashes in `D3D12Device.CaptureBgra` (`capture.alloc.Reset failed: 0x80004005`) — a concurrent UI-side GPU op (the capture resets a command allocator + touches the GPU fence) racing the render thread's submit/present. The render loop itself works (frames rendered up to the capture); it is the concurrent **UI-side GPU touch** that races. This is the proof — not a regression — of the gate: the default + force-sync paths are byte-identical + unaffected.
- **Async prerequisites (the full ComPtr-ownership migration), each real-GPU iteration + soak-gated:** every UI-side GPU touch must move to / rendezvous with the render thread before async is safe to enable — (1) the `UploadImage` staging handoff + retire-fence (§8); (2) resize / device-lost rendezvous (§9); (3) the capture path (or: quiesce the render thread before capture); (4) device/swapchain **CREATION** migration. Then the fuzzed **GPU soak** (§6/§11, sweeping channel-capacity + reader-stall on real hardware) must be green over the required streak before the default flips on. The in-repo `gate.seam.race` validates the quarantine invariant; the nightly GPU soak is the operational gate for the flip.
