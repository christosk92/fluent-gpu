...
- **Residuals / not yet landed (Step 4b + Step 5 — GPU/soak-gated):** device/swapchain **CREATION** still happens UI-side (the full ComPtr-ownership migration is pending — force-sync makes the split safe meanwhile since the two threads never run concurrently); upload/atlas producer→consumer handoff + the retire-fence for resource freeing (§8); device-lost + resize cross-thread rendezvous (§9); the `seam.race` soak harness (§6, still greenfield); and **Step 5** — the async flip (`Quarantine 0→2`, non-blocking publish so the UI stops waiting) that actually lands the BUG3 smoothness win, gated on a green soak. The default stays single-thread until the soak is green.
