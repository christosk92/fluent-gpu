...
- **Residuals / not yet landed (Step 4b + Step 5 — GPU-iteration + soak-gated):** (1) the `UploadImage` handoff (the async prerequisite above) + the retire-fence for resource freeing (§8); (2) device/swapchain **CREATION** migration to the render thread + the device-lost / resize cross-thread rendezvous (§9); (3) **Step 5** — the async flip (`Quarantine 0→2`, non-blocking publish so the UI stops waiting) that actually lands the BUG3 smoothness win. Each needs real-GPU iteration; the default stays single-thread until the fuzzed GPU soak is green over the required streak. **Enabling async before (1)+(2) would ship the exact races the seam exists to prevent — so it stays gated.**
