...
- **DEFAULT FLIPPED ON — async is the default; env flags removed (2026-07-23).** The async render loop is now the default for every real windowed host (`AppHost` resolves `RenderLoopMode.Async`; a Headless window stays `RenderLoopMode.SingleThread` for deterministic gates). The `FG_RENDER_THREAD` and `FG_RENDER_ASYNC` env flags were **deleted** — there is no runtime toggle. `RenderLoopMode.ForceSync` survives only as an internal `AppHost` constructor override (seam tests/probes). The blockers above landed (UploadImage/EvictImage handoff via `ImageUploadQueue`; resize/device-lost rendezvous via `DeviceLostCoordinator`; render-confined video drain; detached children route through the parent's render thread). The 2026-07-03 dim-composite defect that had held the flip off is fixed by deferring `BindDComp` to the presenting thread's first present (`D3D12Device.cs:626-679`) and re-verified 2026-07-23 with on-screen desktop captures + a 4-minute resize/scroll soak (zero device-lost). Windowed out-of-bounds popups use the in-window clamped fallback under async.
