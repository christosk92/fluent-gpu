...
FluentGpu.WindowsApi today is **9 pillars**, all hand-rolled AOT-clean: Notifications, Credentials, Packaging, Activation, Media (SMTC), Dialogs, Shell, Power, Network. The survey walked every non-plumbing WASDK component (`dev/` impls + `specs/` IDL) and cross-checked each against the shipping pillar code.
...
**Decision.** Localization was the one item the original survey deferred outright (MRTCore, §4: "big lift, low value"). On review the right answer was not *adopt MRT later* but *build a better, smaller thing now*. WASDK's `MRTCore`/`resources.pri`/`ResourceManager` is reflection- and satellite-assembly-adjacent, needs a build-time `.pri` authoring pipeline FluentGpu does not have, stores strings in an opaque binary (not diff-reviewable), and — like RESW — has **no plural/select grammar**. FluentGpu instead ships a from-scratch i18n engine under **`src/FluentGpu.Engine/Localization/`** (platform-agnostic, macOS-port-friendly — it is an *engine* feature, behind a host seam, NOT a `FluentGpu.WindowsApi` pillar). It is implemented, compiles, and is gate-verified (`--loc-probe`, **33/33 [PASS]**, exit 0) and render-verified in a gallery page (English / Polish four-form plurals + gender select / pseudo-loc).
...
**Posture.** Zero new interop beyond the one flat-Win32 locale read; no `Microsoft.WindowsAppSDK`, no CsWinRT, no `Mrm*`; NativeAOT + `TrimMode full` survive. **Effort actually spent: M** (runtime engine + parser + gallery page + probe). **Immediate next step (proposed, not built):** a Roslyn source-generator (the repo already has `src/FluentGpu.SourceGen`) that reads the base-culture JSON at build and emits typo-proof typed key accessors — `static class Strings { static class App { const string Title = "app.title"; } }` — so call sites use `Strings.App.Title` (IntelliSense, compile-checked) instead of stringly-typed keys. It emits `const`s only (no runtime reflection), so it stays AOT-clean; deferred from this pass to keep the runtime engine the deliverable.
...
0. **Localization engine (§3.9) — ✅ DONE this cycle.** Shipped under `src/FluentGpu.Engine/Localization/` + the `L`/`Lf`/`UseLocale` hooks + the gallery page; gate-verified (`--loc-probe` 33/33). Remaining follow-up is the deferred typed-key Roslyn source-generator (proposed in §3.9, not half-built). The numbered items below are the still-unbuilt WindowsApi-pillar candidates.
