...
**Plus one cross-cutting engine feature (not a WindowsApi pillar):** **OS file/folder drop**. The Windows backend's `Win32DropTarget` (`[GeneratedComClass]` `IDropTarget` + `RegisterDragDrop`) bridges an OLE drop into the engine via four `InputHooks.ExternalDrag*` delegates → `InputDispatcher.ExternalDrag*` → `DragDropContext.ExternalBegin`. A `BoxEl.DropTarget` accepting `DropKinds.Files` then receives the drop (payload `FileDropData`) exactly like an in-app drag. Headless gate `e5dragdrop.ext` PASS; gallery: new **File & folder drop** card.
