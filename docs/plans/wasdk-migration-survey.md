...
**Status:** research / planning (no code). **Date:** 2026-06.
...
| — | Storage.Pickers, BadgeNotifications, OAuth2Manager, AppInstance-rich, Restart, EnvironmentManager, MRTCore, CameraCaptureUI, Licensing, AccessControl, DWriteCore, PushNotifications, BackgroundTask, WASDK runtime plumbing | — | — | — | — | — | **SKIP** (§4) |
...
| **MRTCore / resources.pri + PrimaryLanguageOverride** | **defer (big lift, low value).** Reachable only via OS WinRT `ResourceManager` (NOT by reusing WASDK's own `Mrm*` DLL — that would be a posture violation) and carries a build-time `.pri` authoring pipeline FluentGpu lacks (it loads assets from disk). A Spotify-class client is server-localized; only chrome strings + scale/contrast assets benefit. Revisit for a localized Store build. Effort L. |
