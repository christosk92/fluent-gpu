...
> **✅ Animation engine — signals-first rework landed + verified.** The animation file map is now: one POD `AnimValue` slab keyed (node, channel) + the `AnimScheduler` (the class is still named `AnimEngine`, so `AnimEngine` references below stay valid); the old `AnimTrack`/sub-stepped-Euler engine is deleted. `InteractionAnimator` (now the `HoverFade`/`PressFade` channels), the `BrushAnim` ticker (now the `BrushFade` channel), and `ConnectedAnimation` (now `DetachedAnimSlab` + `SceneRecorder.RecordDetached`, behind the default-off `FG_DETACHED_FLY` flag) are subsumed; a declarative `Element` surface (`Transition`/`While*`/`Enter`/`Exit`/`Stagger`/`Layout`) + `UseSpringValue`/`UseAnimatedValue` are live alongside the imperative hooks. Design, now implemented: [`../../plans/animation-engine-rework-design.md`](../../plans/animation-engine-rework-design.md).
...
`BoxEl.{Hover,Pressed}{Fill,BorderColor}` + the eased hover/press progress for ordinary interaction (the slab's
`HoverFade`/`PressFade` channels), and the `FluentGpu.Engine` (`Animation/`) `AnimEngine` (the slab scheduler:
keyframes/springs/stroke-trim) for authored WinUI timelines. Building a
...
- **Phase 7 — animation**: `_anim.Tick` — the one `AnimScheduler` over the slab (interaction and brush are slab
  channels now), plus the scroll/repeat/caret tickers — transform/opacity/presented-size only, **never** relayout.
