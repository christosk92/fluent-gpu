       ...
       └ ZStack
          ...
          └ Row (BoxEl{Direction=0}, MaxWidth≈1600, AlignSelf center)
             ...
             └ ScrollView(RightArea)  { Grow=1 }                        // OWN scroller
                └ VStack
                   ├ Chrome  (BoxEl{ScrollBind=[new(){PinTop=0}]})  // toolbar + column header, pinned
...
Both live in **one** `Chrome` box that is a **FIXED bar above the scroller** (§5), not a scrolling sticky node — it sits at the very top of the right column with nothing above it, so it never moves and the rows scroll beneath it. That is identical to the reference's pinned look but needs no sticky machinery (no sticky `ScrollBinds` entry, no `OnFlag`, no engine change): the chrome is just the first child of the right-column `VStack`, above the `ItemsView` (playlist/liked) or above the outer `ScrollView` (album/single). It always carries its toolbar background (it is always "stuck"), which is the simpler, correct default.
...
- The pin pass `ScrollBindEval.ApplyPin` (the `PinKind` op in `ApplyScrollBindsPinPass`) pins a node only when it is inside the **same** scroller's `ContentNode`. A `VirtualListEl` is *its own* scroll viewport, so a sticky `Chrome` placed as its sibling in an outer VStack would pin against the **outer** scroller while the rows live in the **inner** viewport — the chrome and the rows scroll on different surfaces. Wrong.

So "[sticky chrome] + [virtual rows] + [trailing About/Fans/More-by] in one viewport" is not a drop-in today. Three options:

- **(A) Outer non-virtual `ScrollView` whose content is a VStack of `[Chrome with ScrollBinds=[new(){PinTop=0}]] + [eager track list, Grow=0] + [trailing sections]`.** Clean for sticky (chrome *is* in the outer content → the pin pass `ScrollBindEval.ApplyPin` pins it; rows + trailing scroll under it). But a `Grow=0` `VirtualListEl` measures to its layout's full `ContentExtent` (`ContentExtent(n) = n*Extent`, `VirtualLayout.cs:93`), so it realizes **all** rows — virtualization is defeated. Acceptable only for short lists.
...
- **(C) Additive engine seam: `Header`/`Footer` `Element` slots on `VirtualListEl`** (first/last non-recycled children inside the content node, excluded from the windowed range; a `Header` is `PinTop`-eligible). Cleanest long-term API (mirrors WinUI `ItemsRepeater.Header/Footer`), but it touches the reconciler's realize loop and `ScrollState` content-extent math — a real seam, medium risk.
...
The key insight that removes the need for a `Header`/`Footer` engine seam: **the chrome (toolbar + column header) sits at the very top of the right column with nothing above it, so it doesn't need to scroll-then-stick — it's simply a FIXED bar above the scroller.** A plain sibling, no sticky bind, no `OnFlag`, no engine change. The two compositions:
...
- **`:stuck` restyle lands next frame** (signals-first; the pin pass `ScrollBindEval.ApplyPin` fires `OnFlag` on the `StickyPinned` flip into a signal): design the opaque-fill-when-stuck so a 1-frame translucent flash on first pin is acceptable.
