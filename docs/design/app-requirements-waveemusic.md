...
- **PAL `IVideoPresenter`** (`FluentGpu.Windows/Pal/` → DComp; POD `VideoSurfaceId`): `CreateSurface`, `BindSurfaceHandle(id, dcompSurfaceHandle)` (the surface-handoff + single DRM attach point — the video backend produces the DComp surface handle; supersedes the earlier `GetMediaPlayerSink` sink-pull shape <!-- canon-allow: names the superseded seam method on purpose -->), `Place(id, deviceRect, opacity, z)`, `SetVisible`, `Destroy`, `Commit`. FluentGpu **never touches video pixels** (KEEP — the single best decision: the heaviest continuous work is off our single thread by construction, turning v1's worst limitation into a non-issue).
