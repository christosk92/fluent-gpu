...
| [gpu-renderer.md](./gpu-renderer.md) | The batched 2D renderer: every DrawList opcode's PAYLOAD STRUCT SHAPE (incl. DrawImageCmd union + the as-built 6-field DrawVideoCmd), instance structs, SortKey layout + batch-break rules, render-thread record/batch/tessellate types, the HLSL shader set, color contract, clip mechanism, AA ladder, clean-span reuse rule, the batch-time `ImageRef` UV-resolve branch, video hole-punch, **the AS-BUILT vector path lane** (`FillPath`/`StrokePath`, `PathFlatten`/`PathSweep`/`PathStroker`/`PathTessellator`, `PathRealizationCache`/`PathRealizationKey`, the D3D12 `PathPipeline` TRIANGLELIST lane). |
...
| [threading-render-seam.md](./threading-render-seam.md) | The 3-thread topology + single-writer table, ThreadGuard, SceneFramePublisher (triple-buffer), **SceneFrame/SnapshotColumns/CopyInto POD shape** (authority), QuarantinePolicy + ledger, DrawListArenaRing, render-frame ordering invariant, retire-fence handshake, device-lost rendezvous, backpressure, WorkerPool, phase→thread map + seam build order. Also **owns the production-pacing contract** (§11.1): the `DisplayPhaseGate` (incl. `ArmAtPublish`), the refresh-derived pace constants (`AppHost.DeriveAsyncPaceMs`, `DmManualUpdatePacer.SetIntervalMs`), and the two wake seams it drives — `PlatformWaitRequest.WakeOnDisplayClock` + `IPlatformWindow.WakePresent()` (Win32: `Win32CompositorClock` over `DCompositionWaitForCompositorClock`, a runtime capability probe with a wall-clock fallback). |
...
| **FillPathCmd / StrokePathCmd** (`FillPath`=19 / `StrokePath`=20, AS-BUILT 2026-08) — struct SHAPE + raster (the tessellated path lane: fill/stroke triangle soup, the AA fringe, the `PathPipeline` TRIANGLELIST D3D12 lane) | **gpu-renderer.md §5** (shape + raster) / scene-memory.md §4.1 (enum registration) + §2.4 (`VisualKind.Path`) + §2.2 (`_paths` `ColdSlab<PathSpec>` column) |
| `PathTessellator` / `PathRealizationCache` / `PathGeometryTable` / `PathData` + `PathContentEpoch` (AS-BUILT 2026-08) — the CPU flatten→sweep→stroke→fringe pipeline, the retained realization slab + `PathRealizationKey`, and the required-epoch immutable geometry type | **gpu-renderer.md §5/§5.1** |
| `FillRule` in hit-test (AS-BUILT 2026-08) — a path hit-test shares the SAME `PathData.Rule` the tessellator fills with, not just the baked vertices | **gpu-renderer.md §5.1** (owns the rule) / input-a11y.md (consumer) |
| `FillGradientCmd` | **remains UNBUILT** — no `FillGradientCmd`/`FillGradient` opcode, pipeline, or scene column exists in `src/`; the `DrawOp.FillGradient` name printed in gpu-renderer.md §3.1/§4 is design only |
...
| **DrawVideoCmd** (`DrawVideo` = 17, AS-BUILT 2026-07) — struct SHAPE (6-field record: node-local `Dst` + `Radii` + `int SurfaceId` + `VideoReady` + baked `Transform` + `Opacity`) + the DestOut erase raster, the painter-order emit contract, and the main-canvas-only limitation | **gpu-renderer.md §3.1/§7.3** (enum registration + `VisualKind.Video`: scene-memory.md §2.4/§4.1) |
| DrawVideoCmd — present/crossfade/registry consume logic | media-pipeline.md (+ pal-rhi.md present-tree) |
| **EraseRoundRectCmd** (`EraseRoundRect` = 18, AS-BUILT 2026-08) — struct SHAPE (5-field record: node-local `Rect` + `Radii` + `Strength` + baked `Transform` + `Opacity`) + the DestOut erase raster (DrawVideo's PSO/run class reused verbatim) + the recorder BAND-ORDER table and the drop-spotlight scrim emission | **gpu-renderer.md §7.4** (enum registration: scene-memory.md §4.1; drag/scrim POLICY — `DropTargetVisualPolicy`, `DropTargetSpec.SpotlightWhen`, `SceneStore.SpotlightScrimClip`: input-a11y.md) |
...
| **DrawIconMaskCmd** — struct SHAPE + raster posture (ThemedIcon vector-layer mask; `Rect`+`Tint`+`PathId`+`Transform`+`Opacity`; CPU-rasterized colorless R8 mask rides the glyph atlas/PSO, tinted per-instance; NOT the §5 tessellation lane) | **gpu-renderer.md** (struct/raster) / scene-memory.md (enum registration + `VisualKind.IconLayer` + the `ImageId`-doubles-as-PathId column rule) |
| DrawIconMaskCmd — `IconGeometryTable.Shared` interning side-table + ThemedIcon control/registry/role→token semantics | controls.md (ThemedIcon/registry) / theming.md (IconBase/IconAlt tokens + role→token table) / gpu-renderer.md (`IconGeometryTable` parse/raster ownership) |
...
| **Repaint damage / partial repaint (LANDED 2026-08)** — `RepaintDamageRegion` (≤16 float-DIP rects + `RepaintFullReason`), `RepaintRoute` (FullDirect / FullIntoCanvas / Partial), `ReplayRects` (≤4, pixel-grid-disjoint; exactly 1 on the layered route), the replay-unsafe stream set, decode-time cull halos, the `canvasValid` ledger + its self-heals, and the route-parity (bit-identical canvas vs FullDirect) rule | **gpu-renderer.md §13.1/§13.1a** (contracts + policy) / pal-rhi.md (the `FrameInfo` seam type that carries them) / validation.md (`--repaint-identity` pixel gate + the headless `gate.repaint.*`/`gate.damage.*` set) |
| **`FrameInfo` repaint payload** — `RepaintDamage`, `PublishSequence`, `CarriedFromSeq`, `DrawListHash` (the carry-covers-the-gap test and the blit-only stream fingerprint) | pal-rhi.md (seam shape; per-field semantics on `FrameInfo` itself) / gpu-renderer.md §13.1a (what the backend must DO with them) / threading-render-seam.md (`SceneFramePublisher`'s union-forward carry) |
...
| **Context-request routing** (`ContextRequestTrigger {Pointer, Keyboard, Hold, Invoke}` + `ContextRequestEventArgs.Node`/`Source`; `BoxEl.ClickRequestsContext` → implied `ClickBit` + `InteractionInfo.ClickRequestsContextBit = 1u<<16` discriminator, the `uint HandlerMask` widening, `RequestContextFrom`/`InvokeActivation` dispatch) | input-a11y.md §6.5.1 |
| **Engine-owned scroll** (`ScrollIntegrator` + `SetScrollOffset`/`WriteScrollOffset` are the sole offset/clamp/virtualization mutation path; `ScrollTuning` feel knobs; per-notch distance `max(48, 15%·viewport)`; windowed-regression touch velocity. The old OS-owned `IScrollSource`/`ScrollSourceMux`/`Win32DmScrollSource` seam remains removed. Windows DirectManipulation is a narrower, absolute-deadline-paced touchpad **phase-event producer** with hi-res `WM_POINTERWHEEL` fallback; it never writes scene state.) | input-a11y.md §7B (integrator + producer/owner boundary) / `FluentGpu.Windows` Pal/ (DirectManipulation + wheel fallback) |
| **Pointer gesture ownership** (ONE RELEASE, ONE OWNER: `NearestGestureOwner` = nearest self-or-ancestor with `PressedBit`\|`ClickBit`; a click-owning child terminates the release walk; the same owner keys the double/triple-click chain) | input-a11y.md §6.5 |
| **Hover/press descendant cascade** (`SetHoverDescendants`/`SetPressDescendants`, `IsNestedHoverBoundary`, `FollowsContainer`: reveal crosses a boundary, control-state scale never does; one predicate for both cascades + the reconciler mount seed + `SceneRecorder.nodeInteractive`) | backdrop-effects-animation.md §7 |
