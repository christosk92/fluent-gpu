...
| [gpu-renderer.md](./gpu-renderer.md) | The batched 2D renderer: every DrawList opcode's PAYLOAD STRUCT SHAPE (incl. DrawImageCmd union + the as-built 6-field DrawVideoCmd), instance structs, SortKey layout + batch-break rules, render-thread record/batch/tessellate types, the HLSL shader set, color contract, clip mechanism, AA ladder, clean-span reuse rule, the batch-time `ImageRef` UV-resolve branch, video hole-punch. |
