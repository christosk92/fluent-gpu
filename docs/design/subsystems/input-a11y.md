...
> `LightDismiss` scrim closes the top overlay **in `OnPointerDown` — on the press, not the click** (WinUI
> `ShouldEventCloseFlyout` returns true solely for `XCP_POINTERDOWN`, PointerInputProcessor.cpp:1028-1033), and its
> no-op `OnClick` consumes the eventual release; a `Modal` scrim's `OnPointerDown` is a no-op that simply
...
> `CPopupRoot::OnPointerPressed` sets `Handled = didCloseAPopup`, popup.cpp:5206). This is **device-agnostic**: mouse
> press and touch down both route through the SAME press delegate (§7B), so touch
...
- **As-built — per-node delivery is SUBTREE-scoped (WinUI PointerEntered/Exited parity).** `InputDispatcher.UpdateHoverWithin` (the HoverWithin ancestor-chain diff) delivers the per-node handlers on the subtree edges: an ancestor entering the chain gets its `OnHoverMove` (enter), one leaving it gets its `OnPointerExit` — and the leaf's own exit is suppressed when the new hovered node is inside its subtree. So a wrapper whose interactive CHILD wins the deepest-leaf hit (the `ToolTip.Wrap` trigger around any button) still sees hover enter/leave; leaf-only delivery would starve it. Gate: `gate.input.hover-subtree-enter-exit`.
...
**Pointer activation WALKS to the nearest click-owning ancestor (the activation-ownership rule).** §5's self-hit mask deliberately makes a node hit-testable for reasons that are **not** activation — `DragBit` (`CanDrag`), `CursorBit`, `SelectableTextBit`, `ContextBit` — and the deepest such node wins the hit. Delivering the click to that raw hit node therefore turns any `CanDrag`-only (or cursor-only) child inside a clickable row/card into a **click black hole**: the release fires the child's null click column and the row never activates. So a pointer release resolves the **activation owner** first: the nearest enabled self-or-ancestor carrying `ClickBit` (`ClickRequestsContext` implies `ClickBit`, so one bit covers both activation shapes), and fires THAT node's handler. Same walk shape, and the same first-owner-wins stop, as the §6.5.1 context walk and the middle-release walk — stopping at the first owner is what guarantees a nested clickable and its ancestor never both fire.

**ONE RELEASE, ONE OWNER (canonical).** A pointer release resolves exactly ONE **gesture owner** — the nearest enabled self-or-ancestor carrying `PressedBit` **or** `ClickBit` — and `OnPointerReleased` is delivered only when that owner took the press/release half. A **click-owning child therefore terminates the walk with no release**: it owns the gesture outright, and an ancestor's press/release handler is not part of it. The walk formerly tested `PressedBit` alone, which classified an `OnClick`-only child alongside the genuinely inert ones (`CanDrag` label, cursor-only glyph, selectable run) and let the release sail past it — so the two walks ran on the same release with **no shared handled state** and a nested control owned the click while its ancestor owned the release. Shipped consequence: double-clicking a track row's expand chevron toggled its drawer *and* played the track, because the row read `ClickCount = 2` as its double-tap-to-invoke. Inert children are unaffected — those bits are hit-testability, not activation ownership. The same owner resolution keys the **double/triple-click chain**: two presses inside `DoubleClickMs` and the 4 px slop promote only when they resolve to the same owner, since a node-agnostic counter cannot distinguish "the user double-clicked the row" from "the user clicked two adjacent controls quickly" (owner-keyed, not hit-node-keyed, so a plate with inert children stays double-clickable). Gated by VerticalSlice `E2.n` / `E2.o`, both proven failing-first. Three deliberate boundaries: (a) **press-side is not walked** — `_pressed` is the pressed-VISUAL singleton whose ~dozen release/cancel clear-sites key on `_pressed == _down`, so it keeps tracking the raw hit node; the click commit is release-side and no longer gated on it. (b) The mouse **same-target check compares resolved OWNERS, not raw hit nodes** — pressing a row's label and releasing on the row's padding is ONE click on the row, which is the WinUI capture-on-the-owner shape; a release that resolves to no owner (or to a different one) is still not a click, and the hyperlink-span commit stays strictly release-over-the-SAME-node. Touch keeps the strict same-node gate (the §7A pan/slop machinery already owns "did this contact stay put"). (c) The **keyboard, accelerator/access-key, and `_dragTarget` capture-commit** call sites do NOT walk: the first two already hold an activation owner, and the third is a continuous `OnDrag` node whose click handler IS its commit edge — a slider scrub inside a clickable card must never end up clicking the card. Gated by VerticalSlice B.4b.

### 6.5.1 `OnContextRequested` = one declaration, four triggers (WinUI `ContextRequested`)
...
Implementation note (route-walk enrollment): a **cross-axis content-pan Drag** (`DragYieldsToPan` — SwipeControl / FlipView) advertises on the *wrapper* node, but the hit node is often an interactive row *inside* it whose own handler mask carries no drag bit; because `OnDrag`/`OnPointerDown` do not bubble, `TouchDown` walks ancestors from the hit node (or the deepest visual, when nothing interactive was hit) for the nearest enabled `DragYieldsToPan` drag node and enrolls its axis-locked Drag member there, delivering the wrapper's `PointerDown` (its pan anchor) with the wrapper's own model-local point — so a swipe arms and competes with the enclosing scroller's Pan exactly as if the wrapper had been hit directly.
...
- **As-built — inbound OS file/folder drop (with live hover).** The receiving (`RegisterDropTarget`) leg ships as five nullable `InputHooks` delegates — `ExternalDrag{Enter,Over,Leave}` + `ExternalDrop`/`ExternalDropFiles` (the inbound twin of `OpenUri`; `Hooks/Context.cs`), host-wired in the AppHost ctor onto the `InputHooks.Current.Default` channel to `InputDispatcher.ExternalDrag*`/`ExternalDropFiles`. The Windows backend's `Win32DropTarget` (`FluentGpu.Windows/Interop/`) is a **HAND-ROLLED-vtable** OLE `IDropTarget` CCW (`void** Vtbl` first field + static `[UnmanagedCallersOnly(CallConvMemberFunction)]` thunks — the `DWriteItemizer.cs` pattern, NOT source-gen COM: by-value `POINTL` is honored exactly on x64+ARM64; the source-gen attempt crashed on that ABI), registered via `RegisterDragDrop`/`RevokeDragDrop`. **Hover is data-free**: `DragEnter` only `QueryGetData(CF_HDROP)` then forwards `ExternalDragEnter` with EMPTY paths; `DragOver`/`Leave` likewise — so a `BoxEl.DropTarget` accepting `DropKinds.Files` lights up DURING hover (Enter/Over/Leave), the returned engine `DropEffect` drives the OS "+Copy" cursor, and the file list is read ONCE at `Drop` (`GetData(CF_HDROP)` → `DragQueryFileW` → `ExternalDropFiles` fills the `FileDropData` payload before `TryDrop`). The session opens on the same `DragDropContext` (`ExternalBegin`, Source = scene root so `PruneDead` keeps it live), identical to an in-app drag. This delegate-seam shape (not the `IDragDropBackend.RegisterDropTarget` interface above) is the as-built; the interface form remains the canonical macOS-port contract. Gates: `e5dragdrop.ext` (headless seam) + the smoke `6c.1` CCW vtable round-trip (the POINTL-ABI crash guard).
- **As-built — drag/drop STYLING.** Lifted-ghost styling is `DragSource.Style` (`DragVisualStyle{Opacity,Shadow,Scale}`, default = the WinUI 0.80 + flyout shadow; honored in `DragController.Promote`/`ApplyPresented`). Live drag state is reactive via `UseDragState()` (a host-owned `DragEpoch` bumped each frame while a drag is live + `InputHooks.GetDragState`), consumed by `FluentGpu.Controls.DragPreviewLayer` (a cursor-following custom preview) and `FluentGpu.Controls.DropZone` (a drop target that restyles itself — a cross-faded dashed accent ring + accent glow that announces droppability on a compatible live drag and brightens on hover; the dashed ring uses the new `BoxEl.BorderDashOn/Off` channel → `SceneRecorder.StrokeRoundRectDashed`).

- **As-built — the DROP-SPOTLIGHT contract (this doc owns the POLICY; `gpu-renderer.md` §7.4 owns the pixels).** A drop target opts into app-wide deemphasis with `DropTargetSpec.VisualPolicy = DropTargetVisualPolicy.Spotlight`. Off the record path always — `SceneStore.RefreshDropSpotlight(session)`, driven by `DragDropContext` at begin, on every `DropTargetsVersion` change, and once per frame at phase 7.8 (see the refresh-edge bullet below), **never during record** — each opt-in target is capability-tested (kind match → live + not `Disabled` → **hit-REACHABLE** → `CanAccept` → `Transparent` → `SpotlightWhen`) and the survivors are published as the spotlight ROOT set. Reachability mirrors the dispatcher's subtree prune and is proved by the ancestor walk TERMINATING AT `SceneStore.Root` — the only node the hit test descends from — not by running out of ancestors: a KeepAlive-parked page (an inactive tab, which the reconciler parks with `SetSubtreeParked` + `Detach` while retaining `HitTestVisible`, and whose `DropTargetSpec` rows survive because the registry is cleared only on node free) and an exit orphan are both live but unhittable, so neither may advertise a destination or punch a cutout at its stale last-arranged rect. Gate: `e5dragdrop.parked` (plus `e5dragdrop.scrim.reachable` for the cleared-`HitTestVisible` shape). `DropTargetSpec.SpotlightWhen : Func<DragSession,bool>?` is the **per-session** override: a target may opt in generally and still refuse *this* gesture. The canonical use is a same-list reorder — the destination is already under the pointer, so darkening the app the user is reordering inside adds a veil and no information (Wavee's track list sets `SpotlightWhen = s => !IsSameList(s.Payload)`; a cross-list deposit keeps it). **No surviving root ⇒ no scrim band at all.** The set is drag-scoped: `ClearDropSpotlight` on end/cancel/abandon, and a freed node drops out of it in `SceneStore.Free`.
- **As-built — the scrim, and what it replaced.** The deemphasis is ONE recorder band (an opacity group at `DragVisualTok.ScrimOpacity` holding a `DragVisualTok.ScrimColor` veil with one rounded ERASE per spotlight root), emitted **after** the main pass and **before** the drag-ghost / connected-overlay / drag-chip bands. `SceneStore.SpotlightScrimClip : RectF?` scopes it — a persistent scalar the app re-asserts from layout (Wavee publishes its content region, so the title bar and the docked player bar stay lit); null = the whole scene root. The predecessor multiplied the scene root's opacity by `SpotlightBackgroundOpacity = 0.28` and divided it back out on each spotlight subtree; that token, and the presentation-only `SetDropSpotlightExempt`/`IsUnderDropSpotlightExempt` registry it forced (so `DragPreviewLayer`'s chip escaped the dim), are **deleted** — a band above the scrim is lit by construction, and only `RefreshDropSpotlight`/`SetDropSpotlightOver`/`ClearDropSpotlight`/`IsUnderDropSpotlightRoot` + the recorder's `DropSpotlightRootCount`/`DropSpotlightRootAt` iteration survive. `SetDropSpotlightOver` still publishes the HOVERED root; the hover affordance itself remains the target's own (`DropZone`, insertion cue), not a second scrim treatment. Gates: `e5dragdrop.scrim.{cutout,policy,clip,band,cancel,alloc}`. <!-- canon-allow: names the deleted SpotlightBackgroundOpacity token + exempt API on purpose, to record what the scrim replaced -->

- **As-built — the spotlight refresh EDGE is per-frame (phase 7.8), and `DropTargetsVersion` is only a hint.** `SceneStore.DropTargetsVersion` moves when the sparse `DropTargetSpec` column is *written*, so the per-move version gate was never the authority on compatibility: the signals-first bound realize path recycles a row by writing its bind signal (`Reconciler.RebindBoundSlot`), re-pointing a live node at a different logical item **without touching the spec column** — and a `CanAccept` that reads a signal changes its answer with no virtualization at all. The set then went stale IN PLACE: cutouts stayed on the slots that *were* compatible and travelled with them as the rows underneath changed. So a live session also re-collects unconditionally through `DragDropContext.SyncSpotlightBeforeRecord()`, called by the host at **phase 7.8** — after reconcile/layout/realize and the scroll-offset writes, before record — so the scrim's cutouts describe the bindings and the geometry *this* frame paints. Cost: one pass over the realized drop targets per frame, only while a drag is live, 0-alloc (struct enumerator; the root list is cleared, not reallocated). It does **not** request a re-render (the frame is already being recorded).

- **As-built — the LIFT contract (`DragLift`), and the chip that replaced the lifted row.** `DragVisualStyle.Lift` picks between two mutually exclusive drag visuals. `DragLift.Ghost` (default, byte-identical to the pre-chip engine) *is* the source row: `Promote` translates it, dims it to `Opacity`, gives it `Shadow`/`Scale`, composites the subtree as ONE opacity group (`NodePaint.OpacityGroup` — per-primitive alpha let the ghost's own text blend twice against the row beneath it), fills the optional opaque `Backplate` under it (a transparent zebra row otherwise lets content read straight *through* its text), and sets `NodeFlags.DragGhost` so the recorder hoists it into the unclipped ghost band. `DragLift.Stationary` leaves the row in its slot and writes exactly two things — the dim, and `~NodeFlags.HitTestVisible` so drop-target hit-tests see *through* the dragged row — while the moving visual is an independent compact chip rendered by `FluentGpu.Controls.DragPreviewLayer` into the `SceneStore.DragOverlay` band (`controls.md` §7.4). `Shadow`/`Scale`/`Backplate` are Ghost-only. Promotion, every mid-drag re-assert and the restore all route through the same `ApplyPresented`/`RestoreVisuals` pair, so exactly one place knows what a lifted node looks like. **Both modes clamp to the window** (dnd-kit's `restrictToWindowEdges`): Ghost clamps the ghost RECT inside the root's device rect in `RetargetFromRest` (skipped when the dragged node *is* the root, or when the ghost cannot fit on that axis — clamping would pin it and fight the pointer); the chip clamps its measured box inside the same rect in the preview layer's bound transform.

- **As-built — the chip follows on the COMPOSITOR, so the epoch is edge-triggered.** `InputHooks.DragPosX`/`DragPosY` are two engine-owned `FloatSignal`s the host writes from the live session every frame, surfaced as `UseDragPosition()`; a preview BINDS its `Transform` to them, so a pointer move costs one composited transform write — no component render, no reconcile, no layout, 0 alloc. `InputHooks.DragEpoch` (what `UseDragState()` subscribes to) therefore bumps only on the edges a preview's *content* depends on: session begin/end, `OverTarget`, `Effect`, `RefusedTarget`, `Caption`, and the settle window's start/expiry. It used to bump every frame, which re-rendered the whole preview subtree at pointer rate for a value nothing read. Gate: `e5dragdrop.chip.compositor` (drag-move frames re-render zero components and allocate zero bytes).

- **As-built — `SceneStore.DragSourceOpacityOverride`: the DESTINATION owns the source row's dim.** One frame shape has two legitimate owners of the same node's opacity: a same-list insertion performs *virtual removal* (the dragged rows go to 0 — they are "in the chip") while `DragController.ReassertPresented` re-writes the style's 0.4 dim after every mid-drag reconcile, and the re-assert runs after the animation compose. Without a seam the press-source row strobes back to 0.4 on every reconcile frame while its siblings stay hidden. The destination sets this nullable scalar while it hides sources and clears it when it stops; `ApplyPresented` reads `DragSourceOpacityOverride ?? style.Opacity`. Null — the unchanged default for every other drag — means the source's own `DragVisualStyle.Opacity` stands. `DragController.Reset` clears it unconditionally at gesture end, which is the safety net for a destination whose own teardown never ran (a virtualized-away insertion, a torn-down page). The clear lives in `Reset` rather than `RestoreVisuals` deliberately: `Complete`/`Cancel`/`PruneDead` all guard `RestoreVisuals` on the source still being live, which is exactly the `SourceRecycled` case that would otherwise strand the override and hide the NEXT drag's source row entirely. `Reset` is the one chokepoint every exit path funnels through.

- **As-built — dead-source tolerance (`SourceRecycled`) and the `OnAbandoned` abort.** A Ghost drag whose node is freed by a reconcile must abort: its visual literally *was* the corpse. `DragController.PruneDead` resets and — new — fires `OnAbandoned`, which the dispatcher wires to `DragDropContext.Cancel()`; the node's own `OnDragCanceled` column is dead and cannot be invoked, so this is the only notification on that path, and without it the L2 session and its drop spotlight outlived the gesture forever. A **Stationary** drag deliberately survives instead (`DragController.SourceRecycled`): the payload was resolved once at promotion and the chip is the visual, so `PruneDead` returns early, `Move` keeps tracking the pointer while skipping every node-visual write and the node's own handlers, and `DragDropContext.PruneDead` reparents `session.Source` onto the live scene root — exactly the `ExternalBegin` shape an OS drag already had. A virtualized-away row therefore no longer kills a drag that is still on its way to a real drop.

- **As-built — the drop SETTLE window (`DragSettlePhase`).** A Ghost drag settles through `OnSettle`'s FLIP into the node's resting/new slot. A Stationary drag has no lifted node to FLIP, so `DragController` reports the *window* through `OnStationarySettle(phase, target)`: `ToTarget` + the release point when a target accepted, `Home` + the source row's resting rect on refusal/cancel, `None` when there is nowhere to glide (a recycled source nobody accepted). The host latches it and publishes `DragState.Settle`/`SettleTarget` for `AppHost.DragSettleMs` (250ms, the researched "nothing ever teleports" figure), keeping `DragState.Active` **true** across the window over the last live snapshot so the chip can animate out instead of vanishing on the release frame; the payload reference is released when the window expires. A new gesture starting mid-settle cancels the stale one.

- **As-built — TOUCH drag is full L2, at parity with the mouse.** Touch used to be L1-only: `ClaimTouchReorder` promoted the controller, `TouchMove` never called `DragDrop.Move`, and `TouchUp` completed without `TryDrop`/`Cancel` — so a touch drag saw no drop targets, no captions and no edge auto-scroll, and any session opened another way leaked (with its scrim) forever. The three touch sites now mirror the mouse pairing verbatim: the arena's reorder claim opens the session (`DragDrop.TryBegin(Drag.ActiveNode, …, Drag.ActiveLift)` + an immediate `Move`); the `_touchReorder` move branch calls `DragDrop.Move` on the chain under the contact (gated on a live session, so a plain `CanDrag` reorder never pays the extra hit-test walk); and touch-up runs `DragDrop.TryDrop` FIRST — `OnDrop` reads the live session while the visuals are still lifted — then `Drag.Complete(suppressSettle: dropped && !settleGlide, dropped: dropped)`, with the armed-but-never-promoted branch calling `DragDrop.Cancel()`. `CancelPointerContact` cancels L2 before L1 so `OnLeave` fires on a live target while the session still exists.

- **As-built — refusal is PUBLISHED, never silent (`RefusedTarget` / `RefusalCaption` / `DragState.Refused`).** A target that refuses through `CanAccept` is deliberately *transparent* — discovery continues to an accepting ancestor (the Flutter `DragTarget` pass-through) — so it never becomes `OverTarget` and none of its handlers fire. That makes a refusal indistinguishable from empty space, which is precisely how a working feature reads as "drag & drop is broken". So when **nothing on the chain accepts**, `DragDropContext.FindTarget` also reports the NEAREST kind-matched refuser, published as `DragSession.RefusedTarget` with `DropTargetSpec.RefusalCaption(session)` as the session `Caption`; `DragState.Refused` surfaces it and the standard chip renders a not-allowed glyph beside the reason ("Clear sorting to reorder"). It is meaningful only while nothing accepts — an accepting ancestor means the drop still succeeds, so there is nothing to refuse — and it is the distinction `Effect` cannot make, since `DropEffect.None` covers *both* the refusal and the over-nothing case. Hovering empty space stays silent, which is what keeps the glyph meaningful. The cue is resolved after the accepting-target pass so an accepted target's own caption always wins, and re-read per move while refused (return a constant where the reason cannot change mid-gesture). The counterpart seam is `DropTargetSpec.Transparent(session)`: a target it holds for is skipped in `FindTarget` **before** it can be recorded as either acceptor or refusal candidate (and dropped from the spotlight set), so a surface with no business in this gesture — a page body while the user reorders inside its own list, a track table on a non-playlist page — passes the drag through silently instead of accusing it, which is the one thing a `RefusalCaption` cannot do. Gate: `e5dragdrop.transparent`.

- **As-built — drop CAPTIONS (`DragSession.Caption`).** The WinUI `DragUIOverride.Caption` analogue: a target sets `session.Caption` in `OnEnter`/`OnOver` ("Add 3 tracks to Chill", "Play next"), and the engine **clears it on every target change and at session end**, so a target never has to unset its own text. Surfaced through `DragState.Caption` and rendered as the chip's trailing row. Because the clear happens on the change edge, a caption published only on an internal slot edge would vanish the moment the pointer re-entered from a sibling target — targets whose caption depends on the pointer re-assert it in `OnOver` (this is what `Drop.Target<T>(caption:)` and `Reorderable.ForeignCaption` do).

- **As-built — SPRING-LOADING (`SpringLoadDelayMs` / `OnSpringLoad` / `SpringLoadOnly`).** The macOS Finder / WinUI dwell-to-open convention: hold a compatible drag still over a surface for `SpringLoadDelayMs` and `OnSpringLoad(session)` fires **once per Enter**, so a collapsed folder expands or a tab navigates and the user can keep going without dropping first. The dwell accumulates in `DragDropContext.Tick` (which is why `HasActiveWork` now reports the armed window too — the countdown usually runs over a perfectly *stationary* pointer, and an external OS drag emits no L1 gesture to keep frames coming at all); it re-arms only when the dwell host CHANGES, so jitter inside the target does not reset it. **Resolution rule — the spring host is chosen BEFORE acceptance:** the nearest kind-matched target that configures a spring wins, whether it accepts the payload, refuses it through `CanAccept`, or is a pure `SpringLoadOnly` waypoint. A spring-load opens a container; it does not receive a payload — tying it to acceptance would deny it to exactly the surfaces that need it (a folder you cannot drop *into*, a tab that takes no deposit). `SpringLoadOnly` declares that waypoint explicitly: skipped for acceptance AND for the refusal cue, so passing over it neither accepts nor accuses. Gate: `e5dragdrop.springload`.

- **As-built — `Element.BlocksDragArm` (`InteractionInfo.BlocksDragArmBit`, bit 19).** `TryArm` walks UP from the press target for the nearest `DragBit` node, which is what lets a press anywhere on a row drag the row (the WinUI item-container rule). A child that is its own affordance — a card's play FAB, its "…" corner, an inline toggle — sets this bit to STOP that walk at itself. It blocks only the ancestor search: a barrier that is itself draggable still arms, because its own `DragBit` is tested first. Discriminator only — layout, hit-testing and the element's own click/press handling are unaffected, and it is deliberately outside `AnyInteractiveMask`.

- **As-built — the per-source MOUSE drag box (`DragVisualStyle.ThresholdMultiplier`).** `DragController.DragThresholdPx` is the Win32 `SM_CXDRAG`/`SM_CYDRAG` 4px box, tested per-axis. A **click-primary** source — a tab header, a small nav/entity row — is selected far more often than it is dragged, and a mouse that is still travelling when the button goes down routinely crosses 4px before the release, which promotes a drag and *suppresses the click* (the intermittent "the tab didn't select" defect). `DragVisualStyle.ThresholdMultiplier` scales that box per source (`Controls.Drag.Source(thresholdMultiplier:)`, with `Drag.ClickPrimaryThresholdMultiplier = 2` = WinUI's `LISTVIEWBASEITEM_MOUSE_DRAG_THRESHOLD_MULTIPLIER`, `dxaml\xcp\dxaml\lib\ListViewBaseItem_Partial.cpp:54`, whose own comment names this exact bug). Three properties are load-bearing: it is resolved at **ARM** time (`TryArm` reads the armed node's own `DragSource.Style`), because the box is tested in `Move` *before* `Promote` captures the full style — a widened box must apply to the very first move; it is **MOUSE-only** — an `arenaGoverned` touch promotion has already been arbitrated against the arena's own 8px radial `InputDispatcher.TouchSlopPx`, and re-gating it on a widened mouse box would strand a *won* arena below promotion (the two constants are separate by design, exactly as WinUI scopes its own); and values ≤ 0 fall back to the base box (a 0 box would promote on the press). Default 1 ⇒ byte-identical to the unscaled box. Gate: `e5dragdrop.threshold`.

- **As-built — a spring-load waypoint and a real destination are not exclusive.** `SpringLoadOnly` is the opt-*out* of deposit, not the definition of "hover-navigable": a tab that both navigates on dwell **and** accepts a deposit simply leaves `SpringLoadOnly` false and configures `OnSpringLoad` + `OnDrop` together (the dwell host is resolved before acceptance, so both fire from the same hover). The cross-tab shape this enables — dwell to switch pages, or release to file into the tab's own destination — carries one app-level obligation the engine cannot check: a deposit whose target index is **null/append** must refuse a payload that came from the destination itself, or the append arm duplicates the user's own rows back into their own list.

- **As-built — the KEYBOARD/menu move is the a11y answer, not a simulated drag.** Per the Pragmatic model (and §11's posture that AT parity is about *outcomes*), the engine ships no keyboard drag simulation. The accessible equivalent of a reorder is an ordinary command reaching the same mutation seam: in Wavee, **Alt+Up / Alt+Down** moves the selected block one position through the same `MovePlaylistRowsAsync` call and the same pre-move index convention the pointer drop commits with (so a held Alt+Down walks *the same* block down the list), and a context-menu **"Move to playlist…"** reuses the picker + deposit path. `ReorderList`'s `BlockLength` API (`controls.md` §7.4) is what lets the keyboard path move a multi-selection as one unit. Cross-container moves therefore have a full keyboard route without a modal "lift mode".

- **Known limits — drag & drop (recorded, not hidden).**
  - **Touch drag of a `RowSwipe`-wrapped row never promotes.** The swipe recognizer and the drag-reorder recognizer share the contact's arena, and the swipe wins the horizontal-first arbitration; the row is reachable by mouse/pen drag and by the keyboard move above, but a touch drag of a swipeable row is out of reach today. Arena policy, not a bug to patch at the call site.
  - **Touch does not track a `Stationary` source through recycling.** The `_touchReorder` move branch is gated on `_scene.IsLive(_reorderTarget)`, so a touch drag whose source row is virtualized away stops feeding `DragDrop.Move` even though the session itself survives (the mouse path has no such gate). The gesture still completes or cancels correctly — it just stops updating targets until release.
  - **The scrim colour is a single theme-blind constant.** `DragVisualTok.ScrimColor`/`ScrimOpacity` are one pair, because the recorder takes no theme colour beyond focus/scrollbar/text-edit; a light-theme softening needs a new host-plumbed colour (`gpu-renderer.md` §7.4).
  - **A virtualized list's insertion addresses the rows it has.** The declarative insertion (`controls.md` §7.4) resolves slots from the view's own measured bands, so it is exact for realized + overscanned rows; a consumer that pages its model in chunks (Wavee's queue holds ~100 rows) can only reorder within what the model currently holds. Not an engine limit — the geometry is correct for whatever the model exposes — but it is the reason a queue drag cannot reach row 400.
  - **Ghost-lift surfaces without a chip still lift the row.** A `Reorderable` list whose app has not set `DragStyle` (the sidebar customizer / outline surfaces, whose private payload kinds resolve to no chip spec) keeps the historical ghost lift. That is the intended default — `DragStyle` is opt-in precisely so existing single-item surfaces stay byte-identical — but it means two visual languages coexist until every surface opts in.
  - **Unmounting a bound `VirtualListEl` leaks its parked `KeepAlive` slots, drop targets included.** `Reconciler`'s virtual-entry teardown returns the pooled element buffer and fixes the warming/budget censuses, but it never frees the `VirtualEntry.Kept` roots — which were `Detach`ed from the tree when they parked, so the owner's `FreeSubtree` cannot reach them either. Their `DropTargetSpec` rows therefore survive in `SceneStore`'s registry (it is cleared only on node free). This is a **registry/memory** leak, not a visual one: the reachability walk above terminates at `Root`, so a detached parked root can no longer advertise a destination or punch a cutout. Recorded here because the same shape is what made background tabs read as "drag is broken" before that walk was fixed.
...
- **Stuck hover** (content moves under a stationary cursor): hover route invalidated on any topology/z/overlay-push epoch change (§5.4), not just layout commits — re-resolved immediately for a stationary pointer. Two host-driven refreshes realize this at frame end: the scroll-offset-write refresh (a fling/glide moved content under the cursor) and a relayout companion that fires only when the currently-hovered node's own post-transform bounds no longer contain the cursor — a genuine translate-away (a collapsing sidebar carrying its hover-only resize grip out from under a still cursor), NOT a new node merely appearing over the cursor (a tooltip/flyout opening at the pointer, whose dwell hover must survive). Neither synthesizes a hover-enter from an empty hover — a real `PointerMove` still owns first entry.
