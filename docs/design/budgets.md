...
| Backdrop/Effects/Anim | static image baked-blur derivatives | **16 MiB soft budget**, also counted inside the media residency budget; ~1 bake/primary submit and paused during active scroll | unpinned derived images evict LRU-first above the derived soft budget; normal residency eviction/deferred GPU release | pending/failed bake keeps drawing the crisp source image | backdrop-effects-animation.md FA-2b |
| Backdrop/Effects/Anim | baked-blur scratch RTs | fixed **4 MiB**: 2 frames × 2 targets × 512×512×4 B; no per-card/per-frame scratch allocation | fixed for device lifetime; released on device teardown/recovery | target/adoption failure reports failed derivative and retains source fallback | backdrop-effects-animation.md FA-2b |
...
| GPU Renderer | `RepaintDamageRegion` / partial repaint | **≤16 accumulated rects** (world-space float DIP), coalesced to **≤4 replay rects** (1 on the layered route); full redraw if >16 or >60% coverage (checked pre- **and** post-merge) | — | at capacity → least-waste merge, never surrender; over the cutoff → `FullDirect` (today's path, no canvas, no blit) | gpu-renderer.md §13.1/§13.1a |
...
| GPU Renderer | persistent canvas RT | **one** full-window RT (≈5 MB at 1350×960, ≈16 MB at 2496×1592), allocated on the first primary submit and held for the device's life; ONE `canvasValid` ledger, never a per-back-buffer pair | released/recreated only behind a fenced swapchain resize or device re-init | `FullDirect` fallback on >16 rects / >60 % / resize / DPI or clear-colour change / first frame / replay-unsafe stream; per-frame self-heal on instance-bank overflow or an uncovered publish gap | gpu-renderer.md §13.1/§13.1a |
