    ...
    public sealed class SampleExtractorGenerator : IIncrementalGenerator
    {
        ...
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            context.RegisterSourceOutput(collected, static (spc, all) =>
            {
                ...
                foreach (var group in emitted.GroupBy(static m => m.ContainerFqName, StringComparer.Ordinal))
                {
                ...
                // Aggregate registry (the one enumerable list of every sample) — powers the --gallery-audit knob/code
                // coherence check, which can't reflect over the per-page static fields under NativeAOT. Emitted only when
                // this compilation actually declares samples.
                if (emitted.Length > 0)
                    spc.AddSource("GallerySamples.g.cs", SourceText.From(EmitAggregate(emitted), Encoding.UTF8));
            });
        }
        ...
        // FluentGpu.Generated.GallerySamples.All — one (Owner, Sample) row per [Sample] method, referencing the static
        // field the per-container pass emits ({Container}.{Method}Sample). Owner is the container's short name (for audit
        // reporting).
        private static string EmitAggregate(ImmutableArray<SampleModel> members)
        {
            var sb = new StringBuilder();
            sb.Append("// <auto-generated/>\n// Generated by FluentGpu.SourceGen (Gallery/SampleExtractorGenerator) — DO NOT EDIT.\n#nullable enable\n\n");
            sb.Append("namespace FluentGpu.Generated\n{\n");
            sb.Append("    /// <summary>Every [Sample] in the assembly (for the gallery knob/code-coherence audit).</summary>\n");
            sb.Append("    [global::System.CodeDom.Compiler.GeneratedCode(\"FluentGpu.SourceGen\", \"1.0\")]\n");
            sb.Append("    public static partial class GallerySamples\n    {\n");
            sb.Append("        /// <summary>(owner short-name, sample) for every [Sample] method, in stable order.</summary>\n");
            sb.Append("        public static readonly (string Owner, global::FluentGpu.GalleryKit.Sample Sample)[] All = new (string, global::FluentGpu.GalleryKit.Sample)[]\n        {\n");
            foreach (SampleModel m in members
                .OrderBy(static x => x.ContainerFqName, StringComparer.Ordinal)
                .ThenBy(static x => x.MethodName, StringComparer.Ordinal))
            {
                string owner = m.Chain.Length > 0 ? m.Chain[m.Chain.Length - 1].Name : m.ContainerFqName;
                sb.Append("            (").Append(Quote(owner)).Append(", ")
                  .Append(m.ContainerFqName).Append('.').Append(m.MethodName).Append("Sample),\n");
            }
            sb.Append("        };\n    }\n}\n");
            return sb.ToString();
        }
