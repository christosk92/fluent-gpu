...
internal sealed unsafe class ShadowPipeline : IDisposable
{
    ...
    private ShadowInstance* _mapped;
...
""";
    ...
    private void BuildRootSignature(ID3D12Device* device)
    {
        ...
        Check(D3D12SerializeRootSignature(&desc, D3D_ROOT_SIGNATURE_VERSION.D3D_ROOT_SIGNATURE_VERSION_1, &sig, &err), "Shadow.SerializeRootSignature");
        ID3D12RootSignature* rs;
        Check(device->CreateRootSignature(0, sig->GetBufferPointer(), sig->GetBufferSize(), __uuidof<ID3D12RootSignature>(), (void**)&rs), "Shadow.CreateRootSignature");
        _rootSig = rs;
        sig->Release();
    ...
    public void Record(ID3D12GraphicsCommandList* cmd, ReadOnlySpan<ShadowInstance> instances, float vpW, float vpH)
    {
        int start = _cursor;
        ...
        cmd->SetGraphicsRootShaderResourceView(1, _instances->GetGPUVirtualAddress() + (ulong)(start * sizeof(ShadowInstance)));
