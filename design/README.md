...
| [`foundations.md`](./foundations.md) | The architect-of-record shared contracts: handles, allocators, the `SceneStore` SoA columns, DrawList encoding, PAL/RHI/Text seams, frame lifecycle, 18-assembly layout. |
...
> **Scope note.** This is a **v1 architecture**. Deliberate v1 deferrals (each named in the spec): single UI thread (render-thread seam *designed*, not built); grayscale text AA (ClearType v2); display-only text (editing/caret/selection a named follow-up); IME via Imm32 v1 (TSF v2); 2.5D/perspective transforms out (2×3 affine only). None of these change the load-bearing architecture.
...
| P2 | **NativeAOT-clean.** No reflection/`Activator`/`MakeGenericType`/IL-emit, **no `ComWrappers`**. All COM via hand-vtable `calli`. Source generators do all "codegen" at build time. | `<PublishAot>`, `<IsTrimmable>`, zero `[DynamicallyAccessedMembers]` in portable assemblies; `sizoscope`/`.mstat` gate. |
