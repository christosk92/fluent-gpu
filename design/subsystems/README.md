...
| **Scroll source seam** (`IScrollSource`/`IntegratorScrollSource`/`ScrollSourceMux`/`IScrollHost`/`ScrollTuning`/`DmScrollMath`; deterministic integrator default + opt-in Windows DM sample source; per-notch wheel distance `max(48, 15%·viewport)`; windowed-regression touch velocity) | input-a11y.md (seam + integrator + tuning + velocity) / pal-rhi.md (`IPlatformWindow.CreateScrollSource`) / `FluentGpu.Windows` Pal/ (`Win32DmScrollSource`, manual-update DM — confined here, TerraFX) |
