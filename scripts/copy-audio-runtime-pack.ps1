...
param(
    ...
    [string]$PackRoot = (Join-Path $PSScriptRoot "..\app\audio-runtime\packs"),
...
if ($hash -ne "cf95a21911e932cb3a53ae5732c5f8f416b885d79e8cf793107d79e329e9b387") {
    Write-Warning "Hash mismatch - update app/audio-runtime/manifest.json sha256_hex before provisioning."
}

if ($AlsoCopyToWaveeOutput) {
    $outRoots = @(
        (Join-Path $PSScriptRoot "..\app\Wavee\bin\Debug\net10.0\audio-runtime\packs"),
        (Join-Path $PSScriptRoot "..\app\Wavee\bin\Release\net10.0\audio-runtime\packs")
    )
