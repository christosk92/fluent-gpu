...
**File: `app/Wavee/Backend/Resource.cs`** (everything below is playback-agnostic).
...
### Phase 1 tests — new `app/Wavee.Tests/Backend/ResourceTests.cs`
...
### 2a. Head files — `app/Wavee/SpotifyLive/Audio/HeadFileClient.cs`
...
### 2b. Audio keys — `app/Wavee/SpotifyLive/Audio/AudioKeyResolver.cs`
...
### 2c. Storage-resolve — `app/Wavee/SpotifyLive/LiveTrackResolver.cs`
...
`app/Wavee.Tests/Audio/AudioKeyResolverTests.cs` — add:
...
New `app/Wavee.Tests/Audio/HeadFileClientTests.cs` (mock `IHttpExchange`, same fake style as
`FakeLicense`):
...
New `app/Wavee.Tests/Audio/LiveTrackResolverCdnCacheTests.cs` (fake `ITransport` counting
...
**File: `app/Wavee/SpotifyLive/LiveContextResolver.cs`.**
...
Out of scope (unchanged from v1): PlayPlay runtime/license internals (`app/Wavee.PlayPlay/**`,
