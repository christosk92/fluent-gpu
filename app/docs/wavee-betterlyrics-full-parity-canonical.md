...
**The signal surface (alloc-clean), `app/Wavee/Backend/` or `FluentGpu.WindowsApi/Media/`:**
```csharp
...
| B8 FFT + spectrum | `app/Wavee/Backend/` or `src/FluentGpu.WindowsApi/Media/` (`IAudioSpectrum` over `IAudioHost`, `AudioHost.cs:31`); Bar geometry in `Wavee/Features/Player/LyricsView.cs` |
